...
maxHelper y currentMax = max y currentMax
...
slHelper x (currentSum, currentLength) = (currentSum + x, currentLength + 1)
------------------------------------------------------------------------------
...
concatStart = []
concatHelper :: [a] -> [a] -> [a]
concatHelper ys currentList = ys ++ currentList
...
largestHelper x (y:ys) 
  | x > y     = [x]      -- Case 2: If x is greater than the current largest (y), then x is the new sole largest.
  | x == y    = x : y : ys -- Case 3: If x is equal to the current largest (y), add x to the list.
  | otherwise = y : ys   -- Case 4: If x is smaller than the current largest (y), keep the current largest list as is.
...
lastHelper _ (Just y) = Just y  -- already have last, keep it
lastHelper x Nothing  = Just x  -- this is the last element (first encountered)
