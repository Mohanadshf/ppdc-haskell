...
sums :: Int -> [Int]
sums i = sumsHelper i 1 0 -- (target_i, current_n, accumulated_sum)

sumsHelper :: Int -> Int -> Int -> [Int]
sumsHelper target_i current_n current_sum
  | current_n > target_i = [] 
  | otherwise =
      let
        next_sum = current_sum + current_n
      in
        next_sum : sumsHelper target_i (current_n + 1) next_sum
...
sumsOfHelper _ [] = [] 
sumsOfHelper current_total (y:ys) =
  let
    next_total = current_total + y
  in
    next_total : sumsOfHelper next_total ys
------------------------------------------------------------------------------
...
mymaximum bigger currentMax [] = currentMax 
mymaximum bigger currentMax (x:xs) =
  if bigger x currentMax
    then mymaximum bigger x xs 
    else mymaximum bigger currentMax xs
