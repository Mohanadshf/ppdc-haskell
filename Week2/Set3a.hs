...
interpreter commands = interpretHelper 0 0 commands

interpretHelper :: Int -> Int -> [String] -> [String]

interpretHelper x y [] = []
interpretHelper x y (cmd:cmds) =
  case cmd of
    "up"    -> interpretHelper x (y + 1) cmds
    "down"  -> interpretHelper x (y - 1) cmds
    "left"  -> interpretHelper (x - 1) y cmds
    "right" -> interpretHelper (x + 1) y cmds
    "printX" -> show x : interpretHelper x y cmds 
    "printY" -> show y : interpretHelper x y cmds 
    _       -> interpretHelper x y cmds
